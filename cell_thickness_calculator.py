...
# By setting the min_slope, you can ensure that there are no super-thin
# cells that cause the CFL criterion to require a small dt
...
def find_parameters(x1, x2, fx1, fx2, min_slope):
    ...
    # Solve the system
    A, B, C = fsolve(equations, initial_guess, args=(x1, x2, fx1, fx2,
                                                     min_slope))
...
# Adding this function manually to return a list of cell thicknesses
def return_cell_thicknesses(x1, x2, fx2, A, B, C):
    ids = range(x1, x2+1)  # Ids of the cells (should range from e.g., 1 to 50)
    # list of depths /at the bottom of each cell/ 
    z = [A*np.exp(B*id)+C for id in ids]
    # thicknesses calculated with np.diff and rounded to be integers.
    # Prepend=0 ensures another cell at start to that len is correct
    dz = [int(np.round(dz_float)) for dz_float in np.diff(z, prepend=0)]
    # Seeing how far off we are from the target fx2 after rounding and
    # converting to ints
    dfx2 = np.sum(dz) - fx2
    # Adjusting the bottom cell so that the total domain is equal to fx2
    dz[-1] = dz[-1] - dfx2
    return dz
...
if __name__ == "__main__":
    ...
    A, B, C, slope_at_x1, slope_at_x2 = find_parameters(x1, x2, fx1, fx2,
                                                        min_slope)
