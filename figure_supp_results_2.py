...
def results():
    ...
    # ax1
    ax = ax1
    calc_hf(open_dataset(fps('132'))).plot(y='Z', ax=ax, c=colours[0], label='2.2')
    calc_hf(open_dataset(fps('120'))).plot(y='Z', ax=ax, c=colours[1], label='3.1')
    calc_hf(open_dataset(fps('121'))).plot(y='Z', ax=ax, c=colours[2], label='4.0')

    # ax2
    ax = ax2
    calc_hf(open_dataset(fps('139'))).plot(y='Z', ax=ax, c=colours[0], label='2E-3 m$^2$ s$^{-1}$')
    calc_hf(open_dataset(fps('120'))).plot(y='Z', ax=ax, c=colours[1], label='2E-4 m$^2$ s$^{-1}$')
    calc_hf(open_dataset(fps('130'))).plot(y='Z', ax=ax, c=colours[2], label='2E-5 m$^2$ s$^{-1}$')
    calc_hf(open_dataset(fps('122'))).plot(y='Z', ax=ax, c=colours[3], label='2E-6 m$^2$ s$^{-1}$')

    # ax3
    ax = ax3
    (calc_hf(open_dataset(fps('123')))/2).plot(y='Z', ax=ax, c=colours[0], label='200 m')
    calc_hf(open_dataset(fps('130'))).plot(y='Z', ax=ax, c=colours[1], label='100 m')
    (calc_hf(open_dataset(fps('125')))/0.48).plot(y='Z', ax=ax, c=colours[2], label='48 m')

    # ax4
    ax = ax4
    calc_hf(open_dataset(fps('129'))).plot(y='Z', ax=ax, c=colours[0], label='0 W')
    calc_hf(open_dataset(fps('130'))).plot(y='Z', ax=ax, c=colours[1], label='200 W')
    calc_hf(open_dataset(fps('131'))).plot(y='Z', ax=ax, c=colours[2], label='1000 W')

    # ax5
    ax = ax5
    calc_hf(open_dataset(fps('130'))).plot(y='Z', ax=ax, c=colours[0], label='None')
    calc_hf(open_dataset(fps('141'))).plot(y='Z', ax=ax, c=colours[1], label='Bottom +\nsides')
    calc_hf(open_dataset(fps('142'))).plot(y='Z', ax=ax, c=colours[2], label='Bottom\nonly')

    # ax6
    ax = ax6
    calc_hf(open_dataset(fps('140'), dt=8)).plot(y='Z', ax=ax, c=colours[0], label='$dx=$8 m\n45 levels')
    calc_hf(open_dataset(fps('137'))).plot(y='Z', ax=ax, c=colours[1], label='$dx=$4 m\n99 levels')
    calc_hf(open_dataset(fps('138'), dt=3)).plot(y='Z', ax=ax, c=colours[2], label='$dx=$2 m\n198 levels')

    # ax7
    ax = ax7
    calc_hf(open_dataset(fps('135'))).plot(y='Z', ax=ax, c=colours[0], label='1188 m')
    calc_hf(open_dataset(fps('130'))).plot(y='Z', ax=ax, c=colours[1], label='2376 m')
    calc_hf(open_dataset(fps('136'))).plot(y='Z', ax=ax, c=colours[2], label='4752 m')

    # ax8
    ax = ax8
    calc_hf(open_dataset(fps('121'))).plot(y='Z', ax=ax, c=colours[0], label='-1.9 ℃')
    calc_hf(open_dataset(fps('124'))).plot(y='Z', ax=ax, c=colours[1], label='-1.87 ℃')
    calc_hf(open_dataset(fps('134'))).plot(y='Z', ax=ax, c=colours[2], label='-1.84 ℃')

    # ax9
    ax = ax9
    calc_hf(open_dataset(fps('130'))).plot(y='Z', ax=ax, c=colours[0], label='0.037')
    calc_hf(open_dataset(fps('133'))).plot(y='Z', ax=ax, c=colours[1], label='0.034')
    calc_hf(open_dataset(fps('126'))).plot(y='Z', ax=ax, c=colours[2], label='0.031')

    # ax10
    ax = ax10
    calc_hf(open_dataset(fps('144'))).plot(y='Z', ax=ax, c=colours[0], label=r'13.25 ℃')
    calc_hf(open_dataset(fps('130'))).plot(y='Z', ax=ax, c=colours[1], label=r'17.25 ℃')
    calc_hf(open_dataset(fps('143'))).plot(y='Z', ax=ax, c=colours[2], label=r'21.25 ℃')
    ...
    for ax in all_axes:
        ...
        ax.legend(
            ...
            edgecolor='none',
            framealpha=0.68,
            borderpad=0.15,      # padding between text and legend box
